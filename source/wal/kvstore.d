...
class kvstore
{
		...
		out Snapshot* snap , out HardState hs  , out Entry[] entries)
	{
		...
		{
			snap = theAllocator.make!Snapshot(deserialize!Snapshot(snapData));
			log_info(*snap);
		...
		{
			long parse_index;
			log_info(_entrypath , " load " , entryData.length);
			hs = deserialize!HardState(entryData , parse_index);
			log_info(_entrypath , " load " , hs , " " , parse_index , " " , entryData.length);
			if(parse_index < entryData.length)
			{
				entries = deserialize!(Entry[])(entryData[parse_index .. $]);	
				log_info(_entrypath , " load " , entries );
			}
			return true;
		}
		return false;
